...
export default function OwnerResumenPage() {
  ...
  if (!summary) {
    return <p className="text-sm text-gray-400">No se pudo cargar el resumen.</p>;
  }

  return (
    <div className="max-w-2xl">
      <h1 className="text-xl font-semibold mb-6">Resumen del negocio</h1>

      <div className="grid grid-cols-2 gap-3 mb-3">
        <div className="bg-black text-white rounded-2xl p-4">
          <p className="text-xs text-gray-400 mb-1">MRR (suscripciones)</p>
          <p className="text-xl font-semibold">{formatCurrency(summary.mrr)}</p>
          <p className="text-xs text-gray-400 mt-1">{summary.paidAdvisorsCount} asesor(es) en plan pagado</p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-4">
          <p className="text-xs text-gray-400 mb-1">Comisión Referidoo</p>
          <p className="text-xl font-semibold">{formatCurrency(summary.lessioCommissionTotal)}</p>
          <p className="text-xs text-gray-400 mt-1">desde {formatDate(summary.lessioCommissionSince)}</p>
        </div>
      </div>
