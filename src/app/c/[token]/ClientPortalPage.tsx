  ...
  return (
    <>
      <style>{`
        ...
        @keyframes shareHeroPulse {
          0%, 100% { transform: scale(1);    box-shadow: 0 4px 14px rgba(31,174,84,.35); }
          45%       { transform: scale(1.03); box-shadow: 0 8px 26px rgba(31,174,84,.55); }
        }
        .share-hero-cta { animation: shareHeroPulse 2.4s ease-in-out infinite; }
        @media (prefers-reduced-motion: reduce) { .share-hero-cta { animation: none; } }
        @keyframes stepGlow {
            ...
            <div className="px-5" style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + 2rem)" }}>
              ...
              {tab === "inicio" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  {/* Jalón de compartir: héroe accionable solo si aún no ha referido a nadie */}
                  {referrals.length === 0 && (
                    <div style={{ background: "#0d0d0d", borderRadius: 24, padding: 22, color: "#fff" }}>
                      <p style={{ fontSize: 15, fontWeight: 800, letterSpacing: "-.01em", marginBottom: 5 }}>Manda tu enlace y empieza a ganar</p>
                      <p style={{ fontSize: 12.5, color: "rgba(255,255,255,.5)", lineHeight: 1.5, marginBottom: 16 }}>
                        Pásalo por WhatsApp a quien creas que le sirve. Cuando alguien contrate, tú cobras.
                      </p>
                      <button
                        onClick={shareWhatsApp}
                        className="share-hero-cta"
                        style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "#1FAE54", border: "none", borderRadius: 50, padding: "15px 0", fontSize: 15, fontWeight: 700, color: "#fff", cursor: "pointer" }}
                      >
              ...
              {/* Contactar a mi asesor — al fondo, visible desde cualquier pestaña */}
              {advisor.phone && (
                <div style={{ marginTop: 24, paddingTop: 20, borderTop: "1px solid rgba(255,255,255,.08)" }}>
                  <button
                    onClick={() => {
                      const firstName = client.name.split(" ")[0];
                      const digits = advisor.phone!.replace(/\D/g, "").replace(/^(52|1)/, "");
                      const msg = `¡Hola ${advisor.name}! Soy ${firstName}. Tengo una duda sobre mi plan.`;
                      window.open(`https://wa.me/52${digits}?text=${encodeURIComponent(msg)}`, "_blank");
                    }}
                    style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, background: "#1FAE54", border: "none", borderRadius: 50, padding: "14px 0", fontSize: 14, fontWeight: 600, color: "#fff", cursor: "pointer" }}
                  >
                    <svg width="17" height="17" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>
