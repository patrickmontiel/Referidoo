    ...
    <div className="max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-brand-gray-4 mt-0.5">{clients.filter((c) => c.active).length} activos</p>
        </div>
        <div data-tour="actions" className="flex gap-2">
          <input ref={fileRef} type="file" accept=".csv" className="hidden" onChange={handleFileChange} />
          <button
            onClick={() => fileRef.current?.click()}
            className="flex items-center gap-1.5 text-sm px-3 py-2 rounded-full border border-brand-border-4 text-brand-gray-2 hover:bg-brand-surface transition font-medium"
            title="Importar CSV"
          >
          ...
          <button
            ...
            className="flex items-center gap-2 bg-brand-ink text-white text-sm px-4 py-2 rounded-full hover:bg-[#26262a] transition font-medium"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
      ...
      {showForm && (
        <form
          ...
          className="bg-white border border-brand-border-1 rounded-2xl p-5 mb-5 space-y-4"
        >
      ...
      <div data-tour="search" className="relative mb-4">
        <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-brand-gray-4" width="14" height="14" viewBox="0 0 24 24" fill="none">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2.5 border border-brand-border-4 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand-ink transition bg-white"
        />
      </div>
      ...
      {loading ? (
        ...
        <div data-tour="client-list" className="space-y-3">
          {filtered.map((client) => {
            ...
            return (
              <div key={client.id} className="bg-white rounded-2xl border border-brand-border-1 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 bg-brand-border-1 rounded-full flex items-center justify-center text-sm font-semibold flex-shrink-0">
                        {client.name.charAt(0)}
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-sm truncate">{client.name}</p>
                        {client.phone && <p className="text-xs text-brand-gray-4 truncate">{client.phone}</p>}
                      </div>
                    </div>
                  </div>
                  {deactivateId === client.id ? (
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="text-xs px-2.5 py-1 rounded-full bg-red-50 text-red-600 hover:bg-red-100 transition font-medium"
                      >
                        Confirmar
                      </button>
                      <button
                    ...
                    <button
                      ...
                      className="text-brand-gray-4 hover:text-red-400 transition p-1"
                      title="Desactivar"
                    >
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                ...
                <div className="grid grid-cols-3 gap-2 mt-4">
                  <div className="bg-brand-surface rounded-xl p-2.5 text-center">
                    ...
                    <p className="text-[10px] text-brand-gray-4 mt-0.5">Referidos</p>
                  </div>
                  <div className="bg-brand-surface rounded-xl p-2.5 text-center">
                    <p className="text-base font-semibold">{converted}</p>
                    <p className="text-[10px] text-brand-gray-4 mt-0.5">Convertidos</p>
                  </div>
                  <div className="bg-brand-surface rounded-xl p-2.5 text-center">
                    <p className="text-base font-semibold">{formatCurrency(totalPagado)}</p>
                    <p className="text-[10px] text-brand-gray-4 mt-0.5">Pagado</p>
                  </div>
                </div>

                {(escaleraDebido > 0 || escaleraPagado > 0 || burbujaDebido > 0 || burbujaPagado > 0) && (
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    <div className="rounded-xl border border-brand-border-1 p-2.5">
                      <p className="text-[10px] text-brand-gray-4 mb-1.5">Escalera · Vida y PPR</p>
                    ...
                    <div className="rounded-xl border border-brand-border-1 p-2.5">
                      <p className="text-[10px] text-brand-gray-4 mb-1.5">Premios burbuja</p>
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] text-amber-600">Por pagar</span>
                ...
                <div className="mt-4 pt-3 border-t border-brand-border-2 space-y-2">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#1ebe5d] text-white text-sm py-2.5 rounded-full transition font-medium"
                  >
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className="flex-1 flex items-center justify-center gap-1.5 text-xs py-2 rounded-full border border-brand-border-4 text-brand-gray-4 hover:bg-brand-surface transition"
                    >
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-full bg-brand-border-1 text-brand-gray-1 hover:bg-brand-border-4 transition"
                    >
                      Ver dashboard
                    </a>
                  </div>
                ...
                {client.policyNumber && (
                  <p className="text-xs text-brand-gray-4 mt-2">Póliza: {client.policyNumber}</p>
                )}
                <p className="text-xs text-brand-gray-4 mt-0.5">Desde {formatDate(client.createdAt)}</p>
              </div>
            );
          })}
        </div>
