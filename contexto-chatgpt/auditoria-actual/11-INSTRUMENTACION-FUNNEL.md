...
## Verificación
- **200 tests verdes** (33 archivos). Nuevos: `events/route.test.ts` (whitelist + atribución derivada en servidor + dedupe + **no atribución cross-cliente por token, channel arbitrario→null, referralId/PII del browser ignorados**), `activation-funnel.test.ts` (agregación + segregación por asesor), `rate-limit.test.ts`, `limits.test.ts`, `login-rate-limit.test.ts` (429), `login/page.test.tsx` (password no viene de query).
- **E2E real desde estado limpio** (sep-2026, preflight): se vació `ProductEvent` en dev.db y se corrió un recorrido nuevo golpeando los endpoints reales (login asesor QA → crear cliente → portal_link_sent → client_portal_opened ×2 → referral_share_clicked → referral_landing_viewed → referral_form_started → crear referral). Resultado en DB: **7 filas, una por evento**, todas atribuidas server-side al asesor QA; `client_portal_opened` quedó en **1 fila pese a 2 requests** (dedup OK); `referral_created` con su `referralId`; **cero columnas de PII/token**. `/owner/activacion` renderiza exactamente esos conteos (7×1, 3 conversiones limpias en 100%, el resto "—").
- **Visual**: `screenshots-post-funnel/` — funnel agregado + tabla por asesor (con datos del recorrido real, ya no sembrados), perfil corregido (/5), aviso de privacidad corregido.
